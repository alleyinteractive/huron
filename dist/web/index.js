  ...
  _createClass(InsertNodes, [{
    ...
    value: function getMetaFromPath(key, module) {
      ...
      if (-1 !== key.indexOf('./prototypes')) {
        ...
        if (prototype.length) {
          var _prototype = _slicedToArray(prototype, 1);
        ...
        if (testSections && testSections.length && testTypes && testTypes.length) {
          ...
          var _testTypes = testTypes;

          var _testTypes2 = _slicedToArray(_testTypes, 1);
      ...
      if (id && type) {
        ...
        if (renderData) {
          return Object.assign({
            id: id, type: type, key: key, replaceKey: replaceKey, module: module
          }, renderData);
        }
      }

      console.warn('Module \'' + key + '\' does not exist on the page\n      or is no longer in use');
      return false;
    }
